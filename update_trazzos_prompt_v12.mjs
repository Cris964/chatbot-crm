...
const PROMPT_TRAZZOS_V12 = `
# PROMPT MAESTRO — AGENTE IA TRAZZOS (V12 - REGLAS ESTRICTAS DE PRECIO Y CITAS)
...
- Ejemplo: Si acuerdan una cita para el 15 de Junio a las 3 PM, añade al final de tu mensaje: [CITA_AGENDADA: 2026-06-15 15:00]. Esto activará los recordatorios automáticos.
...
- Cuando el cliente pida fotos o catálogo de algún producto, proporciónale el siguiente enlace para que pueda verlas: https://drive.google.com/drive/folders/1Y-gc_eNN8zkBQE7LJ1SIuuWdbJDLoFO5?usp=drive_link
`;

async function updateTrazzosPromptV12() {
    console.log("Updating Trazzos Prompts to V12 (Strict Pricing & Appointment Tracking)...");
    const { error: e1 } = await supabase.from('clients').update({ prompt: PROMPT_TRAZZOS_V12 }).eq('id', TRAZZOS_ID_1);
    const { error: e2 } = await supabase.from('clients').update({ prompt: PROMPT_TRAZZOS_V12 }).eq('id', TRAZZOS_ID_2);
    if (e1 || e2) { console.error("Error:", e1, e2); } 
    else { console.log("✅ Trazzos prompt V12 updated successfully."); }
}

updateTrazzosPromptV12();
