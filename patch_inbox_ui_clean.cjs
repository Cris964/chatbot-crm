...
if (match) {
    ...
    // Instead of completely hacking it with `.replace`, let's do this cleaner.
    ...
    // The old panel ends with `</div>` (the closing of contact-panel)
    // We need to add `</div></div>)}` instead of just the last div.
    newPanel = newPanel.replace(/<\/div>\s*$/, '</div></div></div>)}\n\n      ');
...
console.log('Inbox UI patched cleanly');
