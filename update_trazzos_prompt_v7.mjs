...
const PROMPT_TRAZZOS_V7 = `
# PROMPT MAESTRO — AGENTE IA TRAZZOS (V7 - "CAMI")
...
async function updateTrazzosPromptV7() {
    console.log("Updating Trazzos Prompts to V7...");
    
    await supabase.from('clients').update({ prompt: PROMPT_TRAZZOS_V7 }).eq('id', TRAZZOS_ID_1);
    await supabase.from('clients').update({ prompt: PROMPT_TRAZZOS_V7 }).eq('id', TRAZZOS_ID_2);
    
    console.log("Trazzos prompt V7 updated successfully.");
}

updateTrazzosPromptV7();
