...
export default async function handler(req, res) {
    ...
    // --- LÓGICA DE REASIGNACIÓN (4 HORAS SIN RESPUESTA DEL ASESOR) ---
    console.log("Ejecutando chequeo de reasignación...");
    let reassignmentsCount = 0;
    const { data: humanConvs } = await supabase
      .from('conversations')
      .select('id, client_id, assigned_to, updated_at, messages')
      .eq('needs_human', true)
      .eq('archived', false)
      .not('assigned_to', 'is', null);

    if (humanConvs && humanConvs.length > 0) {
        for (const conv of humanConvs) {
            ...
            if (hoursSinceUpdate >= 4) {
                ...
                if (msgs.length > 0 && msgs[msgs.length - 1].role === 'user') {
                    const { data: teamMembers } = await supabase.from('team_members').select('user_id').eq('client_id', conv.client_id);
                    if (teamMembers && teamMembers.length > 1) {
                        const memberIds = teamMembers.map(m => m.user_id).filter(id => id !== conv.assigned_to);
                        if (memberIds.length > 0) {
                            ...
                            if (convCounts) {
                                convCounts.forEach(c => { if (c.assigned_to) counts[c.assigned_to]++; });
                            }
                            memberIds.sort((a, b) => counts[a] - counts[b]);
                            const newAgentId = memberIds[0];
                            
                            const sysMsg = {
                                role: 'agent',
                                content: '[SISTEMA]: Conversación reasignada automáticamente debido a inactividad (4 horas).',
                                timestamp: new Date().toISOString()
                            };
    ...
    return res.status(200).json({ message: "Cron Hourly completado.", processed: totalProcessed, reassignments: reassignmentsCount });
}
