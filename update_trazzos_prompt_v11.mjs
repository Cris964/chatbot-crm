...
const PROMPT_TRAZZOS_V11 = `
# PROMPT MAESTRO — AGENTE IA TRAZZOS (V11 - "CAMI" CON ESCALAMIENTO)
...
async function updateTrazzosPromptV11() {
    console.log("Updating Trazzos Prompts to V11 (Human Handoff Routing)...");
    const { error: e1 } = await supabase.from('clients').update({ prompt: PROMPT_TRAZZOS_V11 }).eq('id', TRAZZOS_ID_1);
    const { error: e2 } = await supabase.from('clients').update({ prompt: PROMPT_TRAZZOS_V11 }).eq('id', TRAZZOS_ID_2);
    if (e1 || e2) { console.error("Error:", e1, e2); } 
    else { console.log("✅ Trazzos prompt V11 updated successfully."); }
}

updateTrazzosPromptV11();
